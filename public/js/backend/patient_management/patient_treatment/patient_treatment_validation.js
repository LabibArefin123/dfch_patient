...
const TREATMENT_MAX_FILE_SIZE = 10 * 1024 * 1024; // 10 MB

const TREATMENT_ALLOWED_TYPES = [
    "image/jpeg",
    "image/jpg",
    "image/png",
    "image/webp",
...
function validateTreatmentImage(file) {
     ...
     * Check MIME Type
     */
    if (!TREATMENT_ALLOWED_TYPES.includes(file.type)) {
        return {
            valid: false,
            message:
     ...
     * Check File Size
     */
    if (file.size > TREATMENT_MAX_FILE_SIZE) {
        return {
            valid: false,
...
function isTreatmentImage(file) {
    return !!file && TREATMENT_ALLOWED_TYPES.includes(file.type);
}
...
function isTreatmentFileSizeValid(file) {
    return !!file && file.size <= TREATMENT_MAX_FILE_SIZE;
}
